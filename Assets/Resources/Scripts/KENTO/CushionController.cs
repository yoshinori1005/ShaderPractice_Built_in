...
public class CushionController : MonoBehaviour
{
    ...
    {
        // 衝突座標(ワールド座標)
        // ClosestPointOnBoundsは正確な値を返さないので、オブジェクトのバウンズの原点を使用
        ...
        if (LocalPointToUV(hitLocalPos, out var uv))
        {
            ...
            var interactiveZone = new CustomRenderTextureUpdateZone
            {
                ...
                updateZoneSize = new Vector2(
                    boundsSize.x * 1 / planeScale.x * 0.1f,
                    boundsSize.z * 1 / planeScale.z * 0.1f
                    )
            };

            customRenderTexture.SetUpdateZones(
                new CustomRenderTextureUpdateZone[] { defaultZone, interactiveZone }
                );
        }
    }
    ...
    {
        // クリック時のUpdateZoneがクリック後も
        // 適応された状態にならないように一度消去する
        customRenderTexture.ClearUpdateZones();
    }
    ...
    /// <param name="t2">三角形ポリゴンの頂点座標2</param>
    /// <param name="t3">三角形ポリゴンの頂点座標3</param>
    /// <returns同一平面上に存在する任意の座標が三角形内部に存在していたらTrue</returns>
